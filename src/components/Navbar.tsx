import { ArrowUpRightIcon, BikeIcon, ChevronsDownIcon, MapPinIcon, MenuIcon, PackageIcon, SearchIcon, ShoppingCartIcon , UserIcon, XIcon, ShieldIcon, LogOutIcon } from "lucide-react";
import { useState } from "react"
import { useNavigate, Link } from "react-router-dom";

const Navbar = () => {
    const user:any = {name:"john Wick", email:"johnwick@gmail.com", isAdmin:true};
        ...
        <nav className="bg-white sticky top-0 z-50 border-b border-app-border">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center h-16 gap-4">
                ...
                <div className="flex flex-1 items-center justify-end gap-6">
                    ...
                    {/* Search form */}
                    <form  onSubmit={handleSearch} className="hidden sm:flex w-full max-w-sm text-xs sm:text-sm">
                        ...
                        <div className="relative">
                             {user ? (
                                <button onClick={()=> setUserMenuOpen(!userMenuopen)} className="flex items-center gap-2 p-2">
                                  ...
                                  <ChevronsDownIcon  className="size-3 text-zinc-500"/>
                                </button>
                             ) :
                              (
                                <div className="flex-center gap-2">
                                   <Link to="/login" className="hidden md:flex
                                   items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-green-950 rounded-full 
